...
> Date: 2026-06-30 (rev 3 — post ground-truth verification; loop-determinism hardened)
...
US-007 (OMOP target loader) ──────────┐                  │
                       ...
                       US-014 (generality counter-example: no-vocab CSV target — R29 proof)
```
...
- **R29 boundary (architecture-load-bearing):** no `'Maps to'`, `standard_concept`,
  ...
  `compile/`, `targets/loader.py`, the assembler). OMOP/OncoTree specifics live **only**
  in policy objects (US-004) and per-ontology adapter config (US-007). **US-000** makes
  the current tree pass this; **US-001** makes it executable in CI. The denylist's
  **scope of grepped "core paths" explicitly excludes** `eval/`, `tests/`,
  `resolve/policies/`, and `targets/adapters/` (the allowlist), so legitimate
  policy/gold-set/eval references to OncoTree/SNOMED do not trip the guard.
...
- Remove the orphaned **`src/sema/targets/__pycache__/`** (and `targets/adapters/__pycache__/`)
  bytecode as hygiene (no `.py` sources are tracked on `main`; sourceless `.pyc` in
  `__pycache__` is not imported, so this is cleanup, not a shadow-bug fix).
- **PR #109 precondition note (load-bearing for US-004/US-007):** record in
  `progress.txt` that commit `ea2aed7` ("target ontology adapter protocol and manifest
  loader", **never merged to `main`**) contains a full `targets/` tree and a
  `ManifestVocabularyBinding` (`vocabulary, domain, require_standard, allow_zero_default,
  resolver_policy_ref, effective_date_ref`) that overlaps US-004 and US-007. US-004 and
  US-007 **must evaluate recovering it** before reimplementing.
...
- `scripts/check_engine_coupling.py` (or a `tests/` guard) greps a **configured set of
  ...
  `src/sema/resolve/engine.py`, `src/sema/compile/`, `src/sema/targets/loader.py`,
  assembler) for a **policy-owned denylist** of literals; **allowlisted** paths are the
  policy modules (US-004), adapter config (US-007), `eval/`, and `tests/`.
...
Acceptance criteria (+ all Global Standards):
- **PR #109 is reference-only (decision pre-made — no fork):** do **not** attempt to
  recover the unmerged `ea2aed7` branch. Implement the policy object fresh on the
  Slice-0 branch, **mirroring** `ManifestVocabularyBinding`'s field set
  (`vocabulary, domain, require_standard, allow_zero_default, resolver_policy_ref,
  effective_date_ref`) as the schema to copy. Cite `ea2aed7:src/sema/targets/adapters/
  manifest_models.py` in `progress.txt`; do not import or restore it.
...
### US-007 — OMOP target loader (`model_role=TARGET`) from OHDSI field-level CSV · priority 8
**As** the matcher, **I want** OMOP loaded into Neo4j as `model_role=TARGET` entities,
properties, obligations, and vocabulary bindings, **so that** there is a target to map
*to* (architecture §3 step 1; build-vs-buy: parse `OMOP_CDMv5.4_Field_Level.csv`, do not
hand-author). Independent of the resolver — parallelizable.

Acceptance criteria (+ all Global Standards):
- **Source of truth is CSV-parse (decision pre-made — no fork):** parse
  `OMOP_CDMv5.4_Field_Level.csv` directly into `models/planner/target_model.py` classes.
  Do **not** recover the unmerged `ea2aed7` `targets/` tree. Treat PR #109's
  `manifest_*.py` and `ManifestVocabularyBinding` as **reference only** for DTO field
  naming (cite in `progress.txt`). The declarative-manifest "adapter config" path is the
  more vision-aligned long-term direction but is **deferred to a later slice** — it stays
  out of Slice 0 so this story is deterministic.
- Failing tests first: parsing a fixture slice of `OMOP_CDMv5.4_Field_Level.csv`
  (columns `cdmTableName, cdmFieldName, isRequired, cdmDatatype, isPrimaryKey,
  isForeignKey, fkTableName, fkFieldName, fkDomain, fkClass`) yields `TargetObligation` +
  `ForeignKeyObligation` + a `VocabularyBinding` for
  `condition_occurrence.condition_concept_id` with `domain='Condition'` (the `fkDomain`
  column). The binding carries **both** the target Property ref **and** the obligation
  field name (`condition_concept_id`) so the §1.5 property↔field bridge is explicit.
- `src/sema/targets/loader.py` (+ `*_utils.py`, adapter config under an allowlisted
  path): parses the CSV into the existing `models/planner/target_model.py` classes and a
  **new** `VocabularyBinding` node, and materializes `model_role=TARGET` nodes via the
  graph layer (reuse `planner_loader.py` `MAPS_TO`/`DERIVED_FROM` plumbing where it fits).
- Adds the `:VocabularyBinding {domain}` node + `HAS_VOCAB_BINDING` edge (today they do
  not exist on `main`).
- **No OMOP literal in generic loader code** (R29) — `condition_occurrence`, `Condition`,
  etc. come from the parsed CSV / adapter config, never hardcoded in `loader.py`.
- `integration` live test: load the real field-level CSV; assert the
  `condition_concept_id` binding exists with `domain='Condition'` in Neo4j (or, if Neo4j
  is unavailable, against the in-memory materialization plan).
...
- Failing tests first: given a source `:ValueSet` of OncoTree codes, the target
  `condition_concept_id` binding, and a (mocked) resolver, the producer emits one
...
- A `sema` CLI surface for the slice exists: extend `cli.py` with `sema fit` (or `sema
  ...
  push/eval` only and `orchestrate.py` stops after embeddings — this is the first wiring
  of the spine into a single command.)
...
Acceptance criteria (+ all Global Standards):
- Failing tests first: a CSV source (`customers.csv`) → a curated `dim_customer` target
  whose adapter declares obligations (required columns, PK, types) but **no
  `VocabularyBinding`**; the resolver step is **skipped entirely**; every field map uses
  an **existing** `MappingPattern` — `DIRECT_COPY` (copy/rename) and `DERIVED` (a cast
  expression via `DerivedExpression`) and/or `CONSTANT` — with **no `concept_id`, no
  `'Maps to'`, no domain gate**. (Cut: do **not** add `RENAME`/`TYPE_CAST` enum members —
  they do not exist and are unnecessary; existing patterns cover the proof.)
...
- `integration` live test: compile + execute the `dim_customer` write against DuckDB from
  a fixture CSV; assert the rows materialize with no vocabulary columns and the R29 guard
  (US-001) stays green.
...
- **Store-writer ownership:** US-006 is the sole writer of the value-mapping store;
  US-009/US-010/US-011/US-012 read it. PR #109 (`ea2aed7`) is reference-only — do not
  recover it; US-007's source is CSV-parse.
